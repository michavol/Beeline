...
python BLEvaluator.py --config config-files/config_dream4_10.yaml -a -u -j -n -z -y -t -e -x -d -f

# echo "Running config_dream4_100\n"
python BLRunner.py --config config-files/config_dream4_100.yaml
python BLEvaluator.py --config config-files/config_dream4_100.yaml -a -u -j -n -z -y -t -e -x -d -f
...
python BLEvaluator.py --config config-files/config_sergio_bulk_400.yaml -a -u -j -n -z -y -t -d -f -e -x
...
python BLEvaluator.py --config config-files/config_sergio_100.yaml -a -u -j -n -z -y -e -x -f -d -t
