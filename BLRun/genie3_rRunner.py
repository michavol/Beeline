...
ALGORITHM_UPPER = "GENIE3_R"
ALGORITHM_LOWER = "genie3_r"
USER = "18881888"
...
def run(RunnerObj):
    ...
    # Parameters
    nTrees = RunnerObj.params['nTrees']
    nCores = RunnerObj.params['nCores']
    treeMethod = RunnerObj.params['treeMethod']
    K = RunnerObj.params['K']
    ...
    cmdToRun = ' '.join(['docker run --rm -v', str(Path.cwd())+':/data/ '+ USER + '/' + ALGORITHM_LOWER + ':base /bin/sh -c \"time -v -o', "data/" + str(outDir) + 'time.txt', 'Rscript run' + ALGORITHM_UPPER + '.R',
                         inputPath, outPath, str(nTrees), str(nCores), treeMethod, K, '\"'])
    print(cmdToRun)
    os.system(cmdToRun)
